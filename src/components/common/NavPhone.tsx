...
const logo = "https://i.ibb.co/7JvjF6p/company-logo-white.png";
...
const LinksOverlay = ({
  ...
  return (
    <nav className="fixed left-0 top-0 z-40 h-[calc(100vh_-_32px)] w-[calc(100%_-_32px)] overflow-hidden   ">
      <Logo />
      <LinksContainer setActive={setActive} isMenuAvailable={isMenuAvailable} />
      {/* <FooterCTAs /> */}
    </nav>
  );
};
    ...
    <motion.div className="space-y-2 pr-8 pl-12 w-full grid grid-cols-1 justify-center items-end gap-6 pt-36 xl:pt-36  ">
      {LINKS.map((l, idx) => {
        return (
          <NavLink
            key={l.title}
            href={isMenuAvailable ? l.href : l.href2 || "/"}
            idx={idx}
            setActive={setActive}
          >
            {l.title}
...
const NavLink = ({
  ...
  return (
    <motion.button
      ...
      className="w-full xl:w-[45%] flex justify-start text-lg md:text-xl  text-white/60 transition-colors hover:text-white  "
    >
      <span className=" border-[2px] border-white p-[6px] px-3 h-[2.2rem] rounded-full mr-4 text-sm">
        {idx + 1}
      </span>
  ...
  return (
    <>
      <motion.div
        ...
        className={`fixed z-30    top-0 left-0 bg-gradient-dark text-white  `}
      />
...
const LINKS = [
  ...
  },
  {
    ...
    href: "/menu",
    href2:
      "https://drive.google.com/drive/folders/1AMTPLjcivgzJi8U3HU38ydTFefISqNvo?usp=sharing",
  },
  {
    title: "GALLERY",
    href: "/#gallery",
