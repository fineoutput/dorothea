...
export const NavDesktop = ({
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {active && <LinksOverlay isMenuAvailable={isMenuAvailable} />}
      </AnimatePresence>
    </>
  );
};
...
const LinksOverlay = ({
  ...
  return (
    <nav className="fixed left-0 top-0 z-40 h-[calc(100vh_-_32px)] w-[calc(100%_-_32px)] overflow-hidden   ">
      <Logo />
      <LinksContainer isMenuAvailable={isMenuAvailable} />
      {/* <FooterCTAs /> */}
    </nav>
  );
};
...
const NavLink = ({
  ...
  return (
    <>
      ...
      >
        <span className="border-2 border-white p-2 px-3 rounded-full mr-4 text-sm">
          {idx + 1}
        </span>
        {children}
      </motion.a>
...
const Logo = () => {
  ...
  return (
    ...
    >
      <img
        src="https://i.ibb.co/7JvjF6p/company-logo-white.png"
  ...
  return (
    <>
      <motion.div
        ...
        className={`fixed z-30    top-0 left-0 bg-gradient-dark  `}
      />
      {/* <Image
        src={imageNavbar}
        className={`${
